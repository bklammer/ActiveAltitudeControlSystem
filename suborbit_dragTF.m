function [apogee] = suborbit_dragTF(CdA) % [alt, vel, acc]
%-------------------------------------------------------------------------%
% suborbit function
...
%     stages = 1; % Only one stage
    TC1 = evalin('base','M1850'); TC1(1,3) = TC1(1,3)/1000; % Rocket motor
    mass1 = 19.4;   % (kg) rocket mass
    dia = 0.143;    % (m) rocket diameter
...
%         end
        ...
        [alt1,vel1,accel1] = power(launchAlt,0,0,TC1,CdA);
...
% air_brake_impulse = trapz(air_brake_drag)*dt;
% yyaxis left
% plot(t, air_brake_drag);
% ylabel('Increase in Drag (N)');
% yyaxis right
% plot(t, alt);
% ylabel('Altitude (N)');
% xlabel('Time (s)');
% title([num2str(air_brake_fraction*100) '% increase in Drag Coefficient'])
