...
%## rockeflight_2_stage_delivery.m

%## Author: S. Box
%## Created: 2008-05-08


function varargout=rocketflight_2_stage_delivery(varargin);
  ...
  address=varargin{10};%Coordinates of desired landing position
  rsen=varargin{11};%Sensitivity of the landing radius(m)
  imax=varargin{12};%maximum number of iterations
  ...
  apogee_US(1:3)=z_US(end,1:3);
  apogee_BS(1:3)=z_BS(end,1:3);

  Landing_US(1)=interp1(z_USD(:,3),z_USD(:,1),0.2);
  Landing_US(2)=interp1(z_USD(:,3),z_USD(:,2),0.2);

  Landing_BS(1)=interp1(z_BSD(:,3),z_BSD(:,1),0.2);
  Landing_BS(2)=interp1(z_BSD(:,3),z_BSD(:,2),0.2);

  %********************************************************************
      %********************************************************************

      Landstore{i}=Landing_US;

      vec=address-Landing_US;
      if i==1;
  ...
  varargout={El,Az,Asc_TR,Asc_BS,Asc_US,Des_BS,Des_US,apogee_BS,apogee_US,Landing_BS,Landing_US};
